...
describe('weektoweek reports', function ( ) {
  ...
  it ('should produce week to week report in a negative offset timezone', function (done) {
    ...
    client.init(function afterInit ( ) {
      ...
      var startMoment = window.dayjs(startDateString).startOf('day');
      var endMoment = window.dayjs(endDateString).endOf('day');
      ...
      for (var i = 0; i < numberOfDays; i++) {
        var requestStartMoment = window.dayjs.tz(window.dayjs.tz(startDateString, browserTimezone).startOf('day'), profileTimezone).add(i, 'days');
        var requestEndMoment = window.dayjs.tz(window.dayjs.tz(startDateString, browserTimezone).startOf('day'), profileTimezone).add(i + 1, 'days');
