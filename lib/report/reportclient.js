  ...
  client.init(function loaded () {
    ...
    function show (event) {
      ...
      function datefilter () {
        if ($('#rp_enabledate').is(':checked')) {
          ...
          timerange = '&find[created_at][$gte]=' + moment(from).startOf('day').tz(zone).toISOString() + '&find[created_at][$lt]=' + moment(to).endOf('day').tz(zone).toISOString();
      ...
      function dataLoadedCallback (day) {
        ...
        if (loadeddays === dayscount) {
          ...
          var dFrom = moment(sorteddaystoshow[0]);
          var dTo = moment(sorteddaystoshow[(sorteddaystoshow.length - 1)]);
    ...
    function loadData (day, options, callback) {
      ...
      if (client.sbx.data.profile.getTimezone()) {
        from = moment(day).startOf('day').tz(client.sbx.data.profile.getTimezone());
      } else {
      ...
      function loadCGMData () {
        ...
        var query = '?find[date][$gte]=' + from.format('x') + '&find[date][$lt]=' + to.format('x') + '&count=10000';
      ...
      function loadTreatmentData () {
        ...
        var tquery = '?find[created_at][$gte]=' + from.toISOString() + '&find[created_at][$lt]=' + to.toISOString() + '&count=1000';
      ...
      function loadDevicestatusData () {
        ...
        if (options.iob || options.cob || options.openAps || options.predicted) {
          ...
          var tquery = '?find[created_at][$gte]=' + from.toISOString() + '&find[created_at][$lt]=' + to.toISOString() + '&count=10000';
    ...
    function loadProfileSwitch (from, callback) {
      ...
      var tquery = '?find[eventType]=Profile Switch' + '&find[created_at][$lte]=' + from.toISOString() + '&count=1';
      $.ajax('/api/v1/treatments.json' + tquery, {
        headers: client.headers()
        , success: function(xhr) {
    ...
    function loadProfilesRangeCore (dateFrom, dateTo) {
      ...
      var tquery = '?find[startDate][$gte]=' + dateFrom.toISOString() + '&find[startDate][$lte]=' + dateTo.toISOString() + '&sort[startDate]=-1&count=1000';
    ...
    function loadProfilesRangePrevious (dateFrom) {
      ...
      var tquery = '?find[startDate][$lt]=' + dateFrom.toISOString() + '&sort[startDate]=-1&count=1';
    ...
    function loadProfilesRangeNext (dateTo) {
      ...
      var tquery = '?find[startDate][$gt]=' + dateTo.toISOString() + '&sort[startDate]=1&count=1';
